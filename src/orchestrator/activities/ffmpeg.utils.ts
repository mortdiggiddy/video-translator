 ...
 *
 * Provides utilities for extracting audio from video files using ffmpeg.
 */
...
/**
 * Process video URL/path and return local audio file path
 *
 ...
 */
export async function processMediaInput(input: string): Promise<string> {
  ensureTempDir()
  ...
  if (isVideo) {
    ...
    return await extractAudioFromVideo(localPath, audioPath)
  }
